...
- [ ] T002 [P] Add formatted-message fixture builders and mixed-content payload samples in `tests/api/conftest.py` and `web/__tests__/mocks/handlers.ts`
...
- [ ] T009 [P] [US1] Add backend contract and parser tests for supported markdown segmentation in `tests/api/test_conversations.py` and `tests/api/test_formatting_service.py`; include an explicit assertion that segment `text` values containing backslash sequences (e.g. `\[`, `\(`, `\prod`) are stored and returned verbatim without double-escaping or stripping
...
- [ ] T016 [P] [US2] Add backend tests for asset-payload classification and prose-plus-attachment ordering in `tests/api/test_formatting_service.py` and `tests/api/test_conversations.py`
...
- [ ] T021 [US2] Extend the T002 fixtures with structured payload samples (asset-pointer dicts, mixed prose-plus-payload messages) for API and browser tests in `tests/api/conftest.py` and `web/__tests__/mocks/handlers.ts`
- [ ] T022 [US2] Verify the segmented message response remains aligned with the API contract in `tests/api/test_conversations.py` and `web/__tests__/services/api.test.ts`
...
- [ ] T023 [P] [US3] Add backend tests for malformed markdown, unsupported payloads, and missing metadata fallbacks in `tests/api/test_formatting_service.py` and `tests/api/test_conversations.py`
...
- [ ] T035 Add source-parity fixtures and regression coverage for generated assistant content versus imported historical content in `tests/api/conftest.py`, `tests/api/test_formatting_service.py`, and `web/__tests__/mocks/handlers.ts`
- [ ] T036 Add backend and frontend tests for raw HTML and unsafe embedded-content payload handling in `tests/api/test_formatting_service.py`, `tests/api/test_conversations.py`, and `web/__tests__/components/MarkdownRenderer.test.tsx`
...
```text
Task: T009 Add backend contract and parser tests in tests/api/test_conversations.py and tests/api/test_formatting_service.py
...
```text
Task: T016 Add backend asset-payload ordering tests in tests/api/test_formatting_service.py and tests/api/test_conversations.py
...
```text
Task: T023 Add backend fallback tests in tests/api/test_formatting_service.py and tests/api/test_conversations.py
