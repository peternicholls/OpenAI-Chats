...
## Phase 8: Visual Review Fixes (Sprint A — 21 April 2026)

**Purpose**: Address bugs and regressions identified in `visual-review-2026-04-21.md`. Scope is Sprint A only: citation token leaks, unresolved file placeholders, ThinkingBlock scoping, image bubble layout, and sort-button accessibility label. H1 (duplicate turns from conversation branches) is tracked under spec 005 and excluded from this phase.

- [ ] T029 Strip bracket-style citation tokens from rendered message text in `web/src/components/conversations/MarkdownRenderer.tsx`: add `BRACKET_CITATION_RE = /【\d+†L\d+-L\d+】/g` and apply it inside the existing `preprocess()` function alongside `CITATION_RE`; add fixture assertions to `web/__tests__/components/MarkdownRenderer.test.tsx` confirming the pattern is absent from rendered output and surrounding prose is preserved (H2 from visual review)
- [ ] T030 Replace `{{file:file-XXXX}}` placeholders in message content with a readable fallback in `web/src/components/conversations/MarkdownRenderer.tsx`: add `FILE_PLACEHOLDER_RE = /\{\{file:[A-Za-z0-9_-]+\}\}/g` to the `preprocess()` step and substitute `[Referenced file (unavailable)]`; add fixture assertions to `web/__tests__/components/MarkdownRenderer.test.tsx` confirming the raw token is absent and the fallback text is present (H3 from visual review)
- [ ] T031 Scope ThinkingSegment rendering to assistant turns only: in `api/services/archive_service.py` only prepend the pending cluster's `ThinkingSegment` when `msg["author_role"] == "assistant"` — silently discard the cluster when the next visible turn is a user or system message; add a defensive frontend guard in `web/src/components/conversations/MessageBubble.tsx` to skip any segment with `kind === "thinking"` when `message.role !== "assistant"`; add tests in `tests/api/test_conversations.py` asserting a processing cluster followed by a user message produces no thinking segment on that user message, and in `web/__tests__/components/MessageBubble.test.tsx` asserting thinking segments are not rendered for user-role messages (H4 from visual review)
- [ ] T032 Fix attachment image presentation in user message bubbles: in `web/src/components/conversations/AttachmentImage.tsx` increase thumbnail size from `h-32 w-48` to `h-40 w-64`; in `web/src/components/conversations/MessageBubble.tsx` wrap attachment-only and attachment-leading content in `inline-flex flex-wrap gap-2` so the user bubble shrinks to fit its content instead of stretching to full column width; update relevant snapshot or layout assertions in `web/__tests__/components/MessageBubble.test.tsx` (H5 from visual review)
- [ ] T033 Fix sort-button `aria-label` to describe current state rather than action in `web/src/components/layout/SidebarConversationList.tsx`: change the label from `"Sort oldest first"` / `"Sort newest first"` (action) to `"Sorted newest first"` / `"Sorted oldest first"` (current state) to match accessibility convention for toggle controls; update the corresponding aria-label assertions in the sidebar conversation list tests (M10 from visual review)
- [ ] T034 Validate Sprint A changes: run `cd web && npx tsc --noEmit`, `cd web && npm run test`, and `cd web && npm run build`; verify H2 through H5 and M10 are resolved in the browser; update `specs/004-frontend-formatting/remediation-report.md` with Sprint A completion status
