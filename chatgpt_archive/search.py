...
def sanitize_query(query: str) -> str:
    ...
    has_fts5_syntax = fts5_operators.search(query) or '"' in query or "*" in query
...
def _parse_date(date_str: str, start_of_day: bool = True) -> float:
    ...
    try:
        ...
        raise InvalidQueryError(f"Invalid date format: '{date_str}'. Use YYYY-MM-DD.")
...
def execute_search(
    ...
    try:
        ...
        if "fts5" in error_msg.lower() or "syntax" in error_msg.lower():
            raise InvalidQueryError(f"Invalid search query syntax: {query}") from e
        raise
...
def execute_semantic_search(
    ...
    if vec_loaded:
        results = _vec_semantic_search(
            conn, query_blob, query, from_date, to_date, limit
        )
    else:
        results = _fallback_semantic_search(
            conn, query_embeddings[0], query, from_date, to_date, limit
        )
...
def _vec_semantic_search(
    ...
    try:
        ...
        return _fallback_semantic_search(
            conn, deserialize_embedding(query_blob), query, from_date, to_date, limit
        )
